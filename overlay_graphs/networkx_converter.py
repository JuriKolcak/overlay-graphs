...
def _add_vertices_to_nx_graph(graph: Union[mod.Graph, mod.Rule.LeftGraph, mod.Rule.RightGraph],
                              nx_graph: nx.Graph, relabel: Callable[[str], str], use_indices):
    for vertex in graph.vertices:
        nx_graph.add_node(vertex.id if use_indices else vertex, label=relabel(vertex.stringLabel))


def _connect_labelled_edge_in_nx_graph(edge: Union[mod.Graph.Edge, mod.Rule.Edge], nx_graph: nx.Graph, index: int) ->\
        int:
...
def _nx_graph_to_mod(graph: nx.Graph, node_dereference: Callable[[Any], int]) -> mod.Graph:
    return mod.graphGMLString(nx_graph_to_gml(graph, node_dereference))


def graph_to_nx_graph(graph: Union[mod.Graph, mod.Rule.LeftGraph, mod.Rule.RightGraph],
                      relabel: Callable[[str], str] = lambda x: x, use_indices: bool = False) -> nx.Graph:
    nx_graph = nx.Graph()
...
def graph_to_unlabeled_edge_nx_graph(graph: mod.Graph, relabel: Callable[[str], str] = lambda x: x) -> nx.Graph:
    nx_graph = nx.Graph()

    vertex_count = graph.numVertices
...
def rule_combined_graph_to_nx_graph(rule: mod.Rule) -> nx.Graph:
    graph = nx.Graph()

    vertex_count = rule.numVertices
...
def nx_graph_to_gml(graph: nx.Graph, node_dereference: Callable[[Any], int] = lambda x: x) -> str:
    out = ['graph [']
...
def get_component_graphs(graph: nx.Graph, node_dereference: Callable[[Any], int] = lambda x: x) ->\
        Dict[mod.Graph, nx.Graph]:
...
def get_rule_component_graphs_with_nx(rule_graph: Union[mod.Rule.LeftGraph, mod.Rule.RightGraph],
                                      relabel: Callable[[str], str] = lambda x: x) -> Dict[mod.Graph, nx.Graph]:
    nx_graph = graph_to_nx_graph(rule_graph, relabel)

    return get_component_graphs(nx_graph, lambda x: x.id)


def get_rule_component_graphs(rule_graph: Union[mod.Rule.LeftGraph, mod.Rule.RightGraph],
                              relabel: Callable[[str], str] = lambda x: x) -> Iterable[mod.Graph]:
    return get_rule_component_graphs_with_nx(rule_graph, relabel).keys()
